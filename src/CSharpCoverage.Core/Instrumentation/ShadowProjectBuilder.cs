...
public static class ShadowProjectBuilder
{
    public static InstrumentResult Run(InstrumentOptions opts)
    {
        ...
        // Copy full tree
        MirrorDirectory(sourceRoot, Path.Combine(shadowRoot, "src"), opts);
        ...
        foreach (var proj in projects)
        {
            ...
            foreach (var cs in Directory.EnumerateFiles(projDir, "*.cs", SearchOption.AllDirectories))
            {
                ...
                if (MatchesExclude(cs, opts.Excludes)) continue;
    ...
    private static bool MatchesExclude(string path, IEnumerable<string> globs)
    {
        foreach (var g in globs)
        {
            var rx = GlobToRegex(g);
            if (System.Text.RegularExpressions.Regex.IsMatch(path.Replace('\\', '/'), rx)) return true;
        }
        return false;
    }
    ...
    private static void MirrorDirectory(string sourceDir, string destDir, InstrumentOptions opts)
    {
    ...
    private static bool IsSkippedDir(string rel)
    {
        ...
        return rel.StartsWith("bin/") || rel.StartsWith("obj/")
            ...
            || rel.StartsWith("_coverage_shadow/");
    }
