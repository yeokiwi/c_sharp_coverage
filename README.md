...
| `--include <glob>`      | instrument, analyze | Allow-list. Repeatable. When present, only `.cs` files matching at least one glob are rewritten — all other source files are mirrored verbatim.        |
| `--include-from <file>` | instrument, analyze | Read include patterns from a text file (one per line, `#` for comments). Combines with any `--include` patterns.                                       |
| `--exclude <glob>`      | instrument, analyze | Deny-list. Repeatable. Applied after `--include`.                                                                                                      |
| `--exclude-from <file>` | instrument, analyze | Read exclude patterns from a file, same format as `--include-from`.                                                                                    |
| `--verbose`             | instrument          | Print which files were rewritten / skipped, and stream MSBuild output.                                                                                 |
...
### Selecting which `.cs` files to instrument

By default `instrument` and `analyze` rewrite **every** eligible `.cs`
file under the target project (skipping `bin/`, `obj/`, generated
`*.Designer.cs` / `*.g.cs`, and any file with an `<auto-generated>`
header). Use `--include` to switch to an allow-list — only files
matching one of the supplied globs are instrumented; everything else
is copied into the shadow verbatim and built unchanged.

A glob is matched against three forms of each candidate file path,
and matching any one is sufficient:

1. The absolute path on disk
2. The path relative to the project's directory (forward-slashed)
3. The basename

That makes plain filenames work without any glob syntax:

```
# instrument only MainWindow.cs and PhotoViewer.cs
coverage instrument PhotoViewerDemo/PhotoViewerDemo.csproj \
  --include MainWindow.cs --include PhotoViewer.cs
```

Glob examples:

```
# all files under the Models folder
coverage instrument MyApp.csproj --include "Models/*.cs"

# anything matching Calculator*.cs anywhere under the project
coverage instrument MyApp.csproj --include "**/Calculator*.cs"

# every .cs file *except* the generated ones (default behavior)
coverage instrument MyApp.csproj --exclude "Generated/*.cs"
```

For larger lists, keep them in a file and reference it with
...
MainWindow.cs
PhotoViewer.cs
# wildcards are fine too
**/Calculator*.cs

coverage instrument MyApp.csproj --include-from files-to-cover.txt
```

`--include` and `--include-from` accumulate; mixing them is fine.
`--exclude` is always applied after the include filter, so excludes
can carve out files that would otherwise have been included.

When either filter is in effect the run prints a summary of how many
files were instrumented and how many were skipped; pass `--verbose`
to see the per-file decision.
